...
var comment  = require('../../models/topic/posts.js');
var code = 'CTAC'
  ...
  if ( auth.check_login(req.session.user, 1) ){
    ...
    if ( auth.check_params(req.params.id) ) {
      ...
      if( req.body.text && req.body.parent_id !== undefined && req.params.id ) {
        ...
        var topic = [ req.params.id, req.session.user.user_id, req.body.text, req.body.parent_id ];
        ...
        if ( regex.isNumber(topic[0])
          ...
          comment.insert(topic, db, function(has_err, data){
          ...
          response.error_generic(res, code + '0104', 'invalid');
        }
      } else {
